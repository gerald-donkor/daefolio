...
import { ArrowUpRight, Asterisk, Download } from 'lucide-react';
  ...
  return <HomeMotion>
    ...
    <main id="main" tabIndex={-1}>
      ...
      <section id="about" tabIndex={-1} className={styles.about}>
        ...
        <div className={styles.aboutContent}><p className={styles.statement}>The interesting part is where design meets code.</p><p>I’m a design engineer based in Ghana. I like being close to the whole thing: figuring out an interface, building it, then tuning the small details that make it feel natural.</p><p>A useful product can have personality. A beautiful website can work beautifully, too. That’s the space I like working in.</p><div className={styles.capabilities}>{[['Interface design', 'Visual direction, prototypes, design systems'], ['Frontend development', 'React, Next.js, TypeScript'], ['Motion & interaction', 'GSAP, creative coding, the details']].map(([title, description]) => <div key={title}><h3>{title}</h3><span>{description}</span></div>)}</div><div className={styles.resumeActions} role="group" aria-label="Résumé options"><a href="/resume/Gerald-Donkor-Design-Engineer-Resume.pdf" target="_blank" rel="noreferrer" aria-label="View résumé in a new tab" title="View resume" data-cursor="link"><span>Résumé</span><ArrowUpRight size={18} strokeWidth={1.7} aria-hidden="true" /></a><a href="/resume/Gerald-Donkor-Design-Engineer-Resume.pdf" download="Gerald-Donkor-Design-Engineer-Resume.pdf" aria-label="Download résumé PDF" title="Download résumé" data-cursor="link"><Download size={18} strokeWidth={1.7} aria-hidden="true" /></a></div><div className={styles.toolbox} aria-labelledby="toolbox-title"><div className={styles.toolboxHeading}><h3 id="toolbox-title">The tools behind the work.</h3><p>From first frame to final deploy.</p></div><div className={styles.toolGroups}>{toolGroups.map(group => <div className={styles.toolGroup} key={group.label}><span>{group.label}</span><ul>{group.tools.map(([name, icon]) => <li key={name}><Image src={`/logos/${icon}.svg`} alt="" width={28} height={28} aria-hidden="true" /><span>{name}</span></li>)}</ul></div>)}</div></div></div>
      </section>
